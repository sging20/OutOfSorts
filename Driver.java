...
public class Driver{
  public static void main(String[]args){
    if(args.length < 2){
      ...
      //usage: java Driver SIZE ALGO [STYLE]
// supported ALGO: bubble,selection,insertion,test
// supported STYLE: random,equal,sorted,reversed
// //args 0 must be size of the Array
    //args 1 must be "bubble" or "test" (unless you add more)
      ...
      if(args.length < 3 || args[2].equals("random")){
       ...
       for(int i=0; i< randData.length; i++){
         randData[i]= randomnumber;
       }
      }else if(args[2].equals("sorted")){
        int rando= rand.nextInt() % 1000;
        for(int i=1; i< randData.length; i++){
          int adder= (int) Math.random() * 99;
          randData[i]= randData[i-1] - adder;
        }
      }else if(args[2].equals("reversed")){
         int randomnum= rand.nextInt() % 1000;
         for(int i= 1; i< randData.length-1; i++){
           randData[i] = randData[i-1] - ((int) Math.random() * 120);
         }
       }
